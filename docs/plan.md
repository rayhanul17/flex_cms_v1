...
| **Hardening** | Post-audit perf/security/UX fixes | Audit-driven fixes that touch multiple phases: media upload size cap, UserController N+1, log archival batching, preview tokens, draft autosave, 2FA via email/SMS OTP, bulk alt-text editor, rate-limit policies, hotlink protection, EF/Mongo backend consistency. See [phase-hardening-test-cases.md](phase-hardening-test-cases.md) for the manual checklist. |
