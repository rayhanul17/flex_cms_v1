...
public class RssController : Controller
{
    ...
    public RssController(IPostService posts, IMemoryCache cache, ISettingsService settings)
    {
        _posts = posts;
        _cache = cache;
        _settings = settings;
    ...
    private async Task<string> BuildRssAsync(CancellationToken ct)
    {
        ...
        // Channel title + description from site settings so the feed matches
        // the site identity. Falls back to generic labels on any settings error.
        var siteName = "Blog";
        var description = "Latest posts";
        try
        {
            var s = await _settings.GetAsync<SiteSettings>("site:general", ct: ct);
            if (!string.IsNullOrWhiteSpace(s.SiteName))
                siteName = s.SiteName + " — Blog";
        ...
        sb.AppendLine($"    <title>{EscapeXml(siteName)}</title>");
        ...
        foreach (var post in posts)
        {
            ...
            var itemDesc = string.IsNullOrEmpty(post.Excerpt) ? "" : EscapeXml(post.Excerpt);
            sb.AppendLine("    <item>");
            ...
            sb.AppendLine($"      <description>{itemDesc}</description>");
            sb.AppendLine("    </item>");
        }
